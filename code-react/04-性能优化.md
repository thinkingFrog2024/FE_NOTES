...
React.memo 是 React 提供的一个 高阶组件（Higher Order Component, HOC） ，它的核心作用是 通过“缓存”组件的渲染结果来跳过不必要的重渲染 。

在 React 的默认行为中，只要父组件重新渲染，子组件无论 props 是否变化都会跟着重新渲染。 React.memo 就是为了打破这个默认规则。

当你用 React.memo 包裹一个组件时，React 会给这个组件打上一个特殊的标签（ REACT\_MEMO\_TYPE ）。

在 render 阶段（ beginWork 过程中），React 会执行如下逻辑：

- 对比 ：它会拿“新的 props”和“旧的 props”进行对比。
- 浅比较（Shallow Comparison） ：默认情况下，它只进行第一层的引用对比（ prevProps === nextProps ）。
- 跳过 ：如果对比结果一致，React 会直接执行 Bailout ，即： 保留该组件上次生成的 Fiber 节点及其子树，不再执行该组件的函数体，也不去 Diff 它的子节点。

<br />

Q: 为什么不给所有组件都加上 React.memo ？ A:

1. 对比也有开销 ：每次都要遍历 props 对象进行浅比较，如果你的组件 props 非常多且复杂，或者组件本身渲染非常快，那么对比的开销可能比直接重渲染还大。
2. 内存开销 ：React 需要在内存中多存储一份旧的 props 引用。
3. 诚实建议 ：只在组件渲染非常昂贵（如复杂的图表、超长列表）或者 props 变动频率极低的情况下使用。
   Q: 它能替代 shouldComponentUpdate 吗？ A: 是的。它就是类组件中 PureComponent 或 shouldComponentUpdate 在函数组件中的对等实现。
