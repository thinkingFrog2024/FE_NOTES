...
1. **判断是否可以跳过（Bailout 优化）**：对比当前节点的旧 props（来自 current 树的 `memoizedProps`）和新 props（`pendingProps`），如果 props、context （通过createContext创建的上下文）都没有变化且子树也没有待处理的更新，直接复用旧的子节点引用，跳过整棵子树的重建
