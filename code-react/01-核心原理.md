...
Q:react内部怎么处理setState的更新？

A:从抽象层面 ，每一次setState都是一次更新

react使用一个链表 来保存每次更新 并且在组件更新的时候 顺序的执行这个链表来更新组件的状态。

并且react在这里 使用的是特殊的环形链表。

对于更新场景 其实一个带有头指针 尾指针的链表就足够了。

```
// 用 tail/head 两个变量来理解（等价于 queue.pending / queue.pending.next）
// tail   = 最后一个元素（queue.pending）
// head   = 第一个元素  （queue.pending.next）

function enqueueUpdate(update) {
  if (tail === null) {
    // 队列空：第一个元素自己指向自己
    update.next = update;
    tail = update;       // tail 和 head 都是它
  } else {
    // 队列非空：把新元素插到 旧队尾 和 队头 之间
    const head = tail.next;        // 通过 tail.next 拿到队头

    update.next = head;            // ① 新元素的 next → 队头
    tail.next = update;            // ② 旧队尾的 next → 新元素
    tail = update;                 // ③ tail 移到新元素（新的队尾）
  }
}
```

整个过程只用了一个变量 `tail`，而 `head` 通过 `tail.next` 免费获得。每次入队只改 3 个指针，O(1)。
